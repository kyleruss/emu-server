...
public:
	...
	int AddUser(int nDevilSquareIndex,int UserIndex);
	int DelUser(int nDevilSquareIndex,int UserIndex);
	int ChangeUserIndex(int nExUserIndex,int nCurrentUserIndex,int nDevilSquareIndex);
	int LeaveDevilSquare(int nDevilSquareIndex,int UserIndex);
	int GetPlayUserCountRightNow(int nDevilSquareIndex);
...
public:
	...
	BOOL m_bFlag;	// 1C
	UINT m_hThread;	// 20
	std::list<DS_TIME> m_EventTime;	// 24
	int m_iCloseTime;	// 30
	int m_iOpenTime;	// 34
	int m_iPlaytime;	// 38
	CDevilSquareGround m_DevilSquareGround[MAX_DEVILSQUARE_GROUND];	// 3C
	int m_BonusScoreTable[MAX_TYPE_PLAYER][MAX_DEVILSQUARE_GROUND];	// 1BA8
	float m_BonusIncValue[MAX_DEVILSQUARE_GROUND];
