...
void CRaklion::Run()
{
	...
	if(this->isUsed == 0 && !today->tm_hour && !today->tm_min && !today->tm_sec)
	{
		...
		{
			if(gObj[i].MapNumber == MAP_INDEX_RAKLION || gObj[i].MapNumber == MAP_INDEX_RAKLIONBOSS)
			{
...
void CRaklion::CheckUserOnRaklionBossMap()
{
	for ( int iCount=OBJ_STARTUSERINDEX;iCount<OBJMAX;iCount++)
	{
		if ( gObj[iCount].Connected == PLAYER_PLAYING &&
			 ...
			 gObj[iCount].MapNumber == MAP_INDEX_RAKLIONBOSS)
		{
...
int CRaklion::CheckEnterRaklion(int iUserIndex)
{
	...
	if ( gObj[iUserIndex].MapNumber != MAP_INDEX_RAKLIONBOSS ) 
	{
...
#include "LogToFile.h"
CLogToFile RAKLION_TEST_LOG("RAKLION_TEST_LOG", ".\\LOG\\EVENT", TRUE);//identical
...
void CRaklion::UserMonsterCountCheck()
{
	...
	{
		if ( gObj[iAllUserCount].MapNumber == MAP_INDEX_RAKLIONBOSS && gObj[iAllUserCount].Type == OBJ_USER && (gObj[iAllUserCount].Authority&2) != 2 )
		{
	...
	{
		if ( gObj[iAllMonsterCount].MapNumber == MAP_INDEX_RAKLIONBOSS && gObj[iAllMonsterCount].Connected == PLAYER_PLAYING && gObj[iAllMonsterCount].Type == OBJ_MONSTER )
		{
