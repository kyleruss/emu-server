		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					Token = GetToken();
					iElementType = (int)TokenNumber;

					Token = GetToken();
					iSuccessRate = (int)TokenNumber;

					Token = GetToken();
					iContinuanceTime = (int)TokenNumber;

					Token = GetToken();
					iIncAndDecType = (int)TokenNumber;

					Token = GetToken();
					iIncAndDecValue = (int)TokenNumber;

					Token = GetToken();
					iNullifiedMagic = (int)TokenNumber;

					Token = GetToken();
					iCharacterClass = (int)TokenNumber;

					Token = GetToken();
					iCharacterLevel = (int)TokenNumber;
...
void TMonsterSkillElement::ForceSkillElement(int iIndex, int iTargetIndex)
{
	...
	switch ( this->m_iElementType )
	{
		...
		case MSE_TYPE_NORMALATTACK:
			...
			break;
		case MSE_TYPE_SPECIAL_IMMUNE:
			this->ApplyElementSpecialImmune(iIndex,iTargetIndex);
			break;
	}
}
...
BOOL TMonsterSkillElement::ApplyElementHP(int iIndex, int iTargetIndex)
{
	...
	if ( this->m_iIncAndDecType != MSE_INCDEC_TYPE_NONE )
	{
		...
		{
			iIncDecValue = (int)((lpTargetObj->Life * this->m_iIncAndDecValue) / 100.0f);
...
BOOL TMonsterSkillElement::ApplyElementDurability(int iIndex, int iTargetIndex)
{
	...
	if ( lpEquipment == NULL || lpEquipment->IsItem() == FALSE || lpEquipment->m_bLuckySet )
		return FALSE;
...
BOOL TMonsterSkillElement::ApplyElementImmuneSkill(int iIndex, int iTargetIndex)
{
	...
	{
		Time = m_iContinuanceTime;
	}

	gObjAddBuffEffect(lpTargetObj,BUFF_MONSTERMAGIC_RESIST,0,0,0,0,Time);
	return FALSE;
}
	...
	if ( gObjCheckUsedBuffEffect(lpTargetObj,BUFF_POISON) == FALSE )
	{
		if ( this->m_iNullifiedSkill  < 1 )
		{
			if ( retResistance(lpTargetObj, 1) == 0 )
			{
				...
				gObjAddBuffEffect(lpTargetObj,BUFF_POISON,ADD_OPTION_REDUCE_HPPERC,3,0,0,m_iIncAndDecValue);
			}
		}
		else
		{
...
BOOL TMonsterSkillElement::ApplyElementSpecialImmune(int iIndex,int iTargetIndex)
{
	LPOBJ lpObj = &gObj[iIndex];
	LPOBJ lpTargetObj = &gObj[iIndex];

	lpTargetObj->m_MonsterSkillElementInfo.m_iSkillElementSpecialImmuneTime = m_iContinuanceTime;
	...
	{
		EffectTime = m_iContinuanceTime;
	}

	gObjAddBuffEffect(lpTargetObj,BUFF_MONSTERMAGIC_RESIST,0,0,0,0,EffectTime);
	gObjAddBuffEffect(lpTargetObj,BUFF_MONCHEMISTRY_RESIST,0,0,0,0,EffectTime);
