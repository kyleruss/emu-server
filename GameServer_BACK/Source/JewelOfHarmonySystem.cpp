...
void CJewelOfHarmonySystem::_InitOption()
{
	...
	this->m_bSystemPrutiyJewel = GetPrivateProfileInt("GameServerInfo", "PurityHarmonyJewel", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_bSystemMixSmeltingStone = GetPrivateProfileInt("GameServerInfo", "MixSmeltingStoneItem", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_bSystemRestoreStrengthen = GetPrivateProfileInt("GameServerInfo", "RestoreStrengthenItem", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_bSystemStrengthenItem = GetPrivateProfileInt("GameServerInfo", "StrengthenItem", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_bSystemSmeltingItem = GetPrivateProfileInt("GameServerInfo", "SmeltItemBySmeltingStone", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));

	this->m_iRatePuritySuccess = GetPrivateProfileInt("GameServerInfo", "PuritySuccessRate", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iZenForPurity = GetPrivateProfileInt("GameServerInfo", "PurityNeedZen", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iRateMixSmeltingStoneNor = GetPrivateProfileInt("GameServerInfo", "SmeltingStoneSuccesRateNormal", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iRateMixSmeltingStoneExt = GetPrivateProfileInt("GameServerInfo", "SmeltingStoneSuccesRateExcellent", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iZenForMixSmeltingStone = GetPrivateProfileInt("GameServerInfo", "SmeltingNeedZen", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iRateStrengthenSuccess = GetPrivateProfileInt("GameServerInfo", "StrengthenSuccessRate", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iRateSmeltingSuccessNor = GetPrivateProfileInt("GameServerInfo", "SmeltingItemSuccessRateNor", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	this->m_iRateSmeltingSuccessExt = GetPrivateProfileInt("GameServerInfo", "SmeltingItemSuccessRateExt", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
}
	...
	while ( true )
	{
		...
		if ( Token == NUMBER )
		{
			...
			while ( true )
			{
				...
				Token = (SMDToken)GetToken();
				p->iRandomWeight = (int)TokenNumber;

				Token = (SMDToken)GetToken();
				p->iRequireLevel = (int)TokenNumber;

				for ( int i=0;i<MAX_JOH_ITEM_OPTION;i++)
				{
					Token = (SMDToken)GetToken();
					p->iItemEffectValue[i] = (int)TokenNumber;

					Token = (SMDToken)GetToken();
					p->iZenForRestore[i] = (int)TokenNumber;
				}
...
void CJewelOfHarmonySystem::PurityJewelOfHarmony(LPOBJ lpObj)
{
	...
	// Chaos Lock was Enabled
		...
		g_ChaosBox.LogChaosItem(lpObj, "JewelOfHarmony][Purity");
...
#if (ENABLE_FIX_HARMONYANCIENT == 0)
...
BOOL CJewelOfHarmonySystem::MakeSmeltingStoneItem(LPOBJ lpObj)
{
	...
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GCMoneySend(lpObj->m_Index, lpObj->Money);
	g_ChaosBox.LogChaosItem(lpObj, "JewelOfHarmony][Smelt Item Mix");
...
BOOL CJewelOfHarmonySystem::RestoreStrengthenItem(LPOBJ lpObj)
{
	...
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GCMoneySend(lpObj->m_Index, lpObj->Money);
	g_ChaosBox.LogChaosItem(lpObj, "JewelOfHarmony][Restore Item");
	LogAddTD("[JewelOfHarmony][Restore Item] - Restore Start");

	LogAddTD("[JewelOfHarmony][Restore Item] Restore Strengtened Item [%s][%s] Name[%s] ItemType[%d] Serial[%d] OptionType[%d] OptionLevel [%d] Money %d-%d",
...
BOOL CJewelOfHarmonySystem::IsEnableToTrade(LPOBJ lpObj) 
{
	BOOL bRet = TRUE;
