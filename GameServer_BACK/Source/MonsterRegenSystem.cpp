...
void CMonsterRegenSystem::LoadScript(const char* lpFileName)	//OK
{
	...
	{
		MsgBox("[MonsterRegenSystem] File %s not found! %d", File, Result.status);
		return;
	}
	// ----
	xml_node MonsterRegenSystem = Document.child("monstergroupregen");
	xml_node SubNode;
	// ----
	for( xml_node Node = MonsterRegenSystem.child("group"); Node; Node = Node.next_sibling() )
	{
		_stMonsterGroupInfo NewGroup;
		NewGroup.iMapNumber = Node.attribute("map").as_int(-1);
		NewGroup.Interval = Node.attribute("interval").as_int(0);
		NewGroup.iBossMonsterIndex = Node.attribute("boss").as_int(0);
		NewGroup.aType = Node.attribute("regentype").as_int(0);
		NewGroup.dwRegenTick = NewGroup.Interval * 1000 + GetTickCount();
		
		for( SubNode = Node.child("spotlist").child("spot"); SubNode; SubNode = SubNode.next_sibling() )
		{
			_stMonsterPosition NewSpot;
			NewSpot.sx = SubNode.attribute("x1").as_int(0);
			NewSpot.sy = SubNode.attribute("x1").as_int(0);
			NewSpot.ex = SubNode.attribute("x1").as_int(0);
			NewSpot.e = SubNode.attribute("x1").as_int(0);
		}
		...
		if( Token == NUMBER )
		{
			...
			if( nType == 0 )
			{
				while( TRUE )
				{
					...
					Token = GetToken();
					m_MonsterGroupInfo[nGourpNumber].Interval = TokenNumber;
					...
					m_MonsterGroupInfo[nGourpNumber].dwRegenTick = m_MonsterGroupInfo[nGourpNumber].Interval * 1000 + GetTickCount();
				}
			}
			else if( nType == 1 )	
			{
			...
			if( this->m_MonsterGroupInfo[i].aType == 1 )
			{
				if( !IsLiveBossState(i) )
				{
					if( dwTick >= this->m_MonsterGroupInfo[i].dwRegenTick )
					{
						...
						this->m_MonsterGroupInfo[i].dwRegenTick = m_MonsterGroupInfo[i].Interval * 1000 + GetTickCount();
					}
				}
				else
				{
...
//006bf9c0
int CMonsterRegenSystem::GetBoxPosition(int mapnumber,int ax,int ay,int aw,int ah,BYTE & mx,BYTE & my)	// strange attr check ...
{
	int count = 100 ; 
	int w;
	int h;
