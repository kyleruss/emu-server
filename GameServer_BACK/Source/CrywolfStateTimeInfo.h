...
public:
	...
	BOOL CheckScheduleTime()
	{
		...
		if ( this->m_iDay != -1
			&& this->m_iDay != iDay)
		{
			CTime tConditionTime = CTime(iYear, (m_iMonth == -1) ? iMonth : m_iMonth, this->m_iDay, 0, 0, 0, -1) ;
		...
		if(this->m_iHour != -1
			&& this->m_iHour != iHour)
		{
			CTime tConditionTime(iYear, (m_iMonth == -1) ? iMonth : m_iMonth, (m_iDay == -1) ? iDay : m_iDay, this->m_iHour, 0, 0, -1) ;
		...
		{
			CTime tConditionTime(iYear, (m_iMonth == -1) ? iMonth : m_iMonth, (m_iDay == -1) ? iDay : m_iDay, (m_iHour == -1) ? iHour : m_iHour, this->m_iMinute, 0, -1) ;
