		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					Token = GetToken();
					btMapNumber = (BYTE)TokenNumber;

					Token = GetToken();
					btDistance = (BYTE)TokenNumber;

					Token = GetToken();
					btX = (BYTE)TokenNumber;

					Token = GetToken();
					btY = (BYTE)TokenNumber;

					Token = GetToken();
					btDir = (BYTE)TokenNumber;
...
BOOL CKanturuMonsterMng::GetPosition(int TableNum, short MapNumber, BYTE& x, BYTE& y)
{
