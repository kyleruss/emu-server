...
#define MAX_BLOOD_CASTLE_BLOCKINFO	1003
...
#define BC_MAP_RANGE(x) ( (x == MAP_INDEX_BLOODCASTLE8) ? TRUE : ( ((x)<MAP_INDEX_BLOODCASTLE1)?FALSE:((x)>MAP_INDEX_BLOODCASTLE7)?FALSE:TRUE ))
...
#define BC_BLOCKINFO(x) ( ((x)<0)?FALSE:((x)>MAX_BLOOD_CASTLE_LEVEL)?FALSE:TRUE  )

extern int g_iBC_ChoasMixSuccessRate[MAX_BLOOD_CASTLE_LEVEL];
extern int g_iBC_ChoasMixMoney[MAX_BLOOD_CASTLE_LEVEL];
...
typedef struct _BLOODCASTLE_BRIDGE
{
	...
	int m_iBC_DOOR_SUBSIST_INDEX;	//204
	float m_iBC_REWARD_MULTIPLY;	//208
	int m_iBC_CASTLE_BLOCKINFO;		//20C
...
public:
	...
	int GetEventMap(int iBridgeIndex);
	int GetBridgeLevel(int iMapNumber);
	int GetItemMap(int iRetMap);
	int GetRewardMap(int iRetMap);
...
public:
	...
	bool m_bBC_EVENT_ENABLE;	// 1044
	int m_iBC_TIME_MIN_OPEN;	// 1048
	int m_iBC_TIME_MIN_PLAY;	// 104C
	int m_iBC_TIME_MIN_REST;	// 1050
	int m_iBC_NORMAL_ITEM_DROP;	// 1054
	int m_iBC_EXCEL_ITEM_DROP;	// 1058
	int m_iBC_MONSTER_REGEN;	// 105C

	std::list<_EVENT_TIME> m_EventTime;	// 1060
};
...
} g_sttBLOODCASTLE_LEVEL[MAX_BLOOD_CASTLE_LEVEL] =
{
	...
	// Blood Castle 8
	0, MAX_CHAR_LEVEL, // DK, DW, Elf
	0, MAX_CHAR_LEVEL // MG, DL
