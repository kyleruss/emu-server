...
void CCastleDeepEvent::CheckSync()
{
	...
	vector<CASTLEDEEP_EVENTTIME>::iterator it;
...
void CCastleDeepEvent::AddMonster(int iAssultType,int iGroup)
{
	...
	vector<CASTLEDEEP_MONSTERINFO>::iterator it2;
	...
	while( true )
	{
		...
		while( true )
		{
			...
			else if(type == 2)
			{
				...
				map<int,vector<CASTLEDEEP_SPAWNTIME>,less<int> >::iterator it = (map<int,vector<CASTLEDEEP_SPAWNTIME>,less<int> >::iterator)this->m_mapSpawnTime.find(iAssultType);
...
using namespace std;
	...
	if(iTICK_MSEC >= 1000)
	{
		...
		for(map<int,int>::iterator it = (map<int,int>::iterator)m_mapRunCheck.begin();it != m_mapRunCheck.end();it++)
		{
...
void CCastleDeepEvent::StartEvent()
{
	...
	m_mapRunCheck.clear();
	vector<CASTLEDEEP_SPAWNTIME>::iterator it2;
	for(it2 = it->second.begin(); it2 != it->second.end(); it2++)
	{
