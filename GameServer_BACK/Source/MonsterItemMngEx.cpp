...
CMonsterItemMngEx g_MonsterItemMngEx;
// -------------------------------------------------------------------------------

//0042ed00	-> 100% (Identical)
CMonsterItemMngEx::CMonsterItemMngEx()
{
	return;
}
// -------------------------------------------------------------------------------
...
BYTE CMonsterItemMngEx::InsertItem(int monsterlevel, int type, int index, int itemlevel, int op1, int op2, int op3)
{
	...
	// ----
	item->Convert(item_type, op1, op2, op3, 0, 0, 0, 0, 0xFF, CURRENT_DB_VERSION);
	// ----
	if( type == 13 && index == 10 )
	{
...
CItem * CMonsterItemMngEx::GetItem(int monsterlevel)
{
	if( monsterlevel > MAX_MONSTER_LEVEL || monsterlevel < 0 )
	{
		return 0;
...
void CMonsterItemMngEx::gObjGiveItemSearch(int monsterlevel, int maxlevel)
{
	int result;
	int incount = 0;
	int type, index;
...
//		LogAdd("error-L3 : Monster Level Overflow~ %s %d", __FILE__, __LINE__ );	//Line: 8
	...
	while(true)
	{
        ...
        // ----
		if( /*IsPremiumItem*/IsCashSpecialItem(ITEMGET(type, index)) == true )
		{
			continue;
		}
		// ----
...
void CMonsterItemMngEx::LoadMonsterItemDropRate()
{
	/*LPSTR script_file		= gDirPath.GetNewPath(FILE_MONSTER_ITEMDROPRATE);
	...
	this->m_bScriptLoaded = false;
	LPSTR File = gDirPath.GetNewPath(FILE_MONSTER_ITEMDROPRATE);
	xml_document Document;
	xml_parse_result Result = Document.load_file(File);
	// ----
	if( Result.status != status_ok )
	{
		MsgBox("[MonsterItemMng] File %s not found!", File);
		return;
	}
	// ----
	xml_node MonsterItemDropRate = Document.child("monsteritemdroprate");
	// ----
	for( xml_node Node = MonsterItemDropRate.child("monster"); Node; Node = Node.next_sibling() )
	{
		int MonsterLevel = Node.attribute("level").as_int();
		// ----
		if( MonsterLevel >= MAX_MONSTER_LEVEL )
		{
			MsgBox("[MonsterItemMng] Monster level >= MAX_MONSTER_LEVEL");
			return;
		}
		// ----
		this->RateData[MonsterLevel].m_MonsterLevel = MonsterLevel;
		this->RateData[MonsterLevel].m_MagicBookRate = Node.attribute("magicbookrate").as_float() * 10000000.0f;
		this->RateData[MonsterLevel].m_JewelOfBlessRate = Node.attribute("blessrate").as_float() * 10000000.0f;
		this->RateData[MonsterLevel].m_JewelOfSoulRate = Node.attribute("soulrate").as_float() * 10000000.0f;
		this->RateData[MonsterLevel].m_JewelOfLifeRate = Node.attribute("liferate").as_float() * 10000000.0f;
		this->RateData[MonsterLevel].m_JewelOfCreationRate = Node.attribute("creationrate").as_float() * 10000000.0f;
		this->RateData[MonsterLevel].m_JewelOfChaosRate = Node.attribute("chaosrate").as_float() * 10000000.0f;
		this->RateData[MonsterLevel].m_NormalItemRate = Node.attribute("normalrate").as_float() * 10000000.0f;
		// ----
		int nSum = this->RateData[MonsterLevel].m_MagicBookRate
			+ this->RateData[MonsterLevel].m_JewelOfBlessRate
			+ this->RateData[MonsterLevel].m_JewelOfSoulRate
...
void CMonsterItemMngEx::MakeJewelItem()
{
	int item_type;
	// ----
	...
	this->m_MonsterJewelItem1->Convert(item_type, 0, 0, 0, 0, 0, 0, 0, -1, 3);
	...
	this->m_MonsterJewelItem2->Convert(item_type, 0, 0, 0, 0, 0, 0, 0, -1, 3);
	...
	this->m_MonsterJewelItem3->Convert(item_type, 0, 0, 0, 0, 0, 0, 0, -1, 3);
	...
	this->m_MonsterJewelItem4->Convert(item_type, 0, 0, 0, 0, 0, 0, 0, -1, 3);
	...
	this->m_MonsterJewelItem5->Convert(item_type, 0, 0, 0, 0, 0, 0, 0, -1, 3);
...
void CMonsterItemMngEx::gObjGiveItemSearchEx(int monsterlevel, int maxlevel)
{
	this->NormalGiveItemSearch(monsterlevel, maxlevel);
	this->CMonsterItemMngEx::MagicBookGiveItemSearch(monsterlevel, maxlevel);
}
// -------------------------------------------------------------------------------

//004309b0	-> 100% (Identical)
void CMonsterItemMngEx::MagicBookGiveItemSearch(int monsterlevel, int maxlevel)
{
	int result	= 0;
	int incount = 0;
	int type, index;
	...
	while(true)
	{
		...
		if( type == 12 && index == 11 )
		{
			...
			if( result >= 0 )
			{
				...
				item->Convert(item_type, 0, 0, 0, 0, 0, 0, 0, -1, 3);
				item->m_Level		= result;
				item->m_Durability	= item->m_BaseDurability;
				// ----
				incount++;
...
void CMonsterItemMngEx::NormalGiveItemSearch(int monsterlevel, int maxlevel)
{
	int result;
	int incount = 0;
	int type, index;
...
int	CMonsterItemMngEx::CheckMonsterDropItem(int type, int index)
{
	if(		(type == 14 && index == 13)
		||	(type == 14 && index == 14)
		||	(type == 14 && index == 16)
...
CItem * CMonsterItemMngEx::GetItemEx(int monsterlevel)
{
	...
	// ----
	if( monsterlevel > MAX_MONSTER_LEVEL-1 ) //fix
	{
		monsterlevel = MAX_MONSTER_LEVEL-1;
	}
	// ----
	if( monsterlevel > MAX_MONSTER_LEVEL || monsterlevel < 0 )
	{
