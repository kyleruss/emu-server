...
BOOL CKanturu::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		this->m_BattleStanby.LoadData(gDirPath.GetNewPath(FILE_EVENT_KANTURU_MAIN));
		this->m_BattleOfMaya.LoadData(gDirPath.GetNewPath(FILE_EVENT_KANTURU_MAIN));
		this->m_BattleOfNightmare.LoadData(gDirPath.GetNewPath(FILE_EVENT_KANTURU_MAIN));
		this->m_TowerOfRefinement.LoadData(gDirPath.GetNewPath(FILE_EVENT_KANTURU_MAIN));
	}
	catch ( DWORD )
	{
...
int CKanturu::CheckEnterKanturu(int iUserIndex)
{
		 ...
		 this->m_BattleOfMaya.GetEntrancePermit() == TRUE)
	{
...
#if (CUSTOM_WINGS == 1)
			 && (gObj[iUserIndex].pInventory[7].m_Type < ITEMGET(12,440) || gObj[iUserIndex].pInventory[7].m_Type > ITEMGET(12,445))
#endif
			 && (gObj[iUserIndex].pInventory[7].m_Type < ITEMGET(12,262) || gObj[iUserIndex].pInventory[7].m_Type > ITEMGET(12,267))
		...
		if( IsTransformEquipment(iUserIndex) )
		{
