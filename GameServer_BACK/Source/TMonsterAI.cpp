...
BOOL TMonsterAI::RunAI(int iIndex, int iMonsterClass)
{
	...
	if ( (GetTickCount() - lpObj->m_iLastAIRunTime) < (DWORD)lpAIUnit->m_iDelayTime )
		return FALSE;
...
BOOL TMonsterAI::UpdateCurrentAIUnit(int iIndex)
{
	...
	{
		UTIL.SendCrywolfChattingMsg(lpObj->m_Index, "UpdateCurrentAI ­ %s Done!", TMonsterAIUnit::FindAIUnit(iOldCurrentAI)->m_szUnitName, TMonsterAIUnit::FindAIUnit(iCurrentAI)->m_szUnitName);
	}
