		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					if ( bVerified ==FALSE )
					{
						LogAddC(2, "[Monster Manager] - Invalid SkillInfo : MIndex(%s/%d) (%s) File.",
...
TMonsterSkillUnit * TMonsterSkillManager::FindMonsterSkillUnit(int iIndex, int iMonsterSkillUnitType)
{
	...
	int iFoundSkillArray[MAX_SKILL_UNIT] = {-1};
...
int gGetLowHPMonster(int nZoneIndex,int nIndex,int nDistance)
{
	...
	int nTargetLife = 0;
	int nMonsterCount = g_ImperialGuardian.GetZoneInfo(nZoneIndex).m_vtMonsterIndexInfo.size();

	for( int i = 0; i < nMonsterCount; i++)
	{
		_stMonsterIndexInfo monsterInfo = g_ImperialGuardian.GetZoneInfo(nZoneIndex).m_vtMonsterIndexInfo.at(i);

		if( gObjCalDistance(lpObj,&gObj[monsterInfo.m_iIndex]) < nDistance &&
			monsterInfo.m_bLiveMonster )
		{
			nTempTargetLife = (gObj[monsterInfo.m_iIndex].Life / gObj[monsterInfo.m_iIndex].MaxLife)*100.0f;

			if( nTempTargetLife < nTargetLife || nTargetLife == 0 )
			{
				if(  nTempTargetLife != 100 && nTempTargetLife > 0 )
				{
					nTargetLife = nTempTargetLife;
					nTargetIndex = monsterInfo.m_iIndex;
				}
			}
		}
	}
...
void TMonsterSkillManager::UseMonsterSkill(int iIndex, int iTargetIndex, int iMonsterSkillUnitType,int iMonsterSkillUnit,CMagicInf* lpMagic)
{
	...
	{
		int iUnit = iMonsterSkillUnit;
		lpMonsterSkillUnit = lpMonsterSkillInfo->m_lpSkillUnitArray[iUnit];
	}
	else
	{
	...
	if ( IF_MAP_RANGE(lpObj->MapNumber) )
	{
		...
		{
			int nResult = gGetLowHPMonster(lpObj->m_ImperialGuardianZone,lpObj->m_Index,6);
			...
			{
				lpTargetObj = &gObj[nResult];
				iTargetIndex = nResult;
			}
		}
		...
		{
			lpMagic = &cMagicInf;
			TMonsterSkillManager::FindMagicInf(lpMonsterSkillUnit,lpMagic);
		}
	}
	...
	if ( lpMonsterSkillUnit->m_iUnitScopeType == -1 )
	{
		...
		//if( lpMonsterSkillUnit->m_iUnitTargetType == -1 )
			//iTargetIndex = iIndex;
