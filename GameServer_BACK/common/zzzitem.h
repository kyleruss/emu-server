...
#define DBI_GET_OPTION16(x) ( ((x) & 0x40) >> 4  )
...
#define DBI_SOCKET1_DATA	11
#define DBI_SOCKET2_DATA	12
#define DBI_SOCKET3_DATA	13
#define DBI_SOCKET4_DATA	14
#define DBI_SOCKET5_DATA	15
...
public:
	...
	int ItemDefense();
	int ItemCurseDamage();
	int IsClass(char aClass, int ChangeUP,int ChangeUP2);
	LPSTR GetName();
	int GetLevel();
	...
	int Is3rdWingOpGetOnePercentDamage();
	int Is25WingOpGetOnePercentDamage();
	int Is3rdLevelWingReflect();
	int Is3rdLevelWingHPRestore();
	int Is25LevelWingHPRestore();
	int Is3rdLevelWingMPRestore();
	...
	void InitSocketItem(BYTE btCount);
	int GetExcOptCount();
	int IsFullAncient();
	bool IsFullExcellent();
	int GetDetailItemType();

#if (ENABLETEST_MUUN == 1) // need compare calls
	bool IsMuunItmePeriodExpire();
	void SetMuunItmePeriodExpire();
	int GetMuunItemRank();
#endif
	...
	short m_PShopBlessValue;
	short m_PShopSoulValue;
	short m_PShopChaosValue;
