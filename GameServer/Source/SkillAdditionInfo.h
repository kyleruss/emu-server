...
public:
	...
	short GetManaShieldRate(int level) { if( level < 0 || level > 20 ) return 20; return m_SB_ManaRate[level]; };

	int GetBloodStormExplosionRate() { return this->m_iBloodStormExplosionRate; }
	int GetBloodStormExplosionDistance() { return this->m_iBloodStormExplosionAttackDistance; }
	int GetBloodStormExplosionDamage() { return this->m_iBloodStormExplosionDamage; }
...
private:
	...
	int m_iInfinityArrowMPConsumptionPlus3;	// 14
	...
	int m_iBloodStormExplosionAttackDistance;
	int m_iBloodStormExplosionRate;
	int m_iBloodStormExplosionDamage;

	WORD m_SB_ManaRate[21];
};
