...
int g_LuckyItemDurabilityTime;	//GameMain.cpp (0b0415e8)
// -------------------------------------------------------------------------------
...
int LuckyItemManager::LoadLuckyItemInfo(char * lpszFileName)	//OK
{
		...
		if( Token == NUMBER )
		{
			...
			else if( Type == 20 )	//-> ItemLevelRandRate
			{
				...
				while(true)
				{
					...
					int iIndex = TokenNumber;
					// ----
					for( int i = 0; i < iTicketCount; i++ )	//-> Error? bcz it will be from 0 to 15........
					{
						Token = GetToken();		
			...
			else if( Type == 21 )	//-> AddOpt3Rate
			{
				...
				while(true)
				{
					...
					int iIndex = TokenNumber;
					// ----
					for( int i = 0; i < iTicketCount; i++ )	//-> too error...
					{
						Token = GetToken();		
	...
	// ----
	fclose(SMDFile);
	this->AddLuckyItemInfo(&LuckyItemInfo[0], iTicketCount);
	// ----
	return true;
}
// -------------------------------------------------------------------------------
...
void LuckyItemManager::AddLuckyItemInfo(_LUCKY_ITEM_INFO * pLuckyItemInfo, int iItemCnt)	//OK
{
	for( int i = 0; i < iItemCnt; i++ )
	{
		...
		memcpy(&LuckyItemInfo, &pLuckyItemInfo[i], sizeof(LuckyItemInfo));
	}
}
// -------------------------------------------------------------------------------
...
{
	std::map<int, _LUCKY_ITEM_INFO>::iterator Iter = LuckyItemInfo.find(iItemNum);
	// ----
...
{
	//2400 = default
	return 2400;//g_LuckyItemDurabilityTime;
}
// -------------------------------------------------------------------------------
...
//005b01f0	-> 100%
int LuckyItemManager::LuckyItemRepaire(LPOBJ lpObj, int source, int target)	//OK
{
...
int LuckyItemManager::GDReqLuckyItemInsert(short wItemCode, long dwSerial, short wDurabilitySmall, int iObjIndex)	//OK
{
	...
	memcpy(pMsg.szCharName, lpObj->Name, sizeof(pMsg.szCharName));
...
void LuckyItemManager::GDReqLuckyItemInsert2nd(int iObjIndex)	//OK
{
	...
	memcpy(pMsg.szCharName, lpObj->Name, sizeof(pMsg.szCharName));
	// ----
 	PHeadSubSetB((LPBYTE)&pMsg, 0xD1, 0x24, sizeof(pMsg));
...
void LuckyItemManager::DGAnsLuckyItemList(PMSG_ANS_LUCKYITEM_SELECT * lpRecv)	//OK
{
	...
	// ----
	int iDeleteItemList[12];
	memcpy(iDeleteItemList, lpRecv, iItemCnt * 12);	//wtf, why 12 * X...
	int iInven = 0;
	// ----
	...
	{
		if( gObj[iObjIndex].pInventory[i].IsItem() )
		{
			CItem pItem = gObj[iObjIndex].pInventory[i];
			// -----
			for( int j = 0; j < iItemCnt; j++ )	//not "j"
			{
				if( pItem.m_Type == iDeleteItemList[6 * j] && pItem.m_Number == iDeleteItemList[3 * j] )
				{
					pItem.m_DurabilitySmall = iDeleteItemList[6 * j];
					iDeleteItemList[j]		= 1;
					break;
				}
			}
	...
	{
		if( iDeleteItemList[i] == 0 )
		{
			this->GDReqLuckyItemDelete(iDeleteItemList[6 * i], iDeleteItemList[3 * i], iObjIndex);
		}
	}
}
// -------------------------------------------------------------------------------
...
//005b0b80	-> 100%
void LuckyItemManager::GDReqLuckyItemSelect(LPOBJ lpObj)	//OK
{
