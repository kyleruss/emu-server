...
void GDGuildMemberAdd(int aIndex, LPSTR Name, LPSTR MemberId)
{
	...
	pMsg.NumberH = SET_NUMBERH(aIndex);
	pMsg.NumberL = SET_NUMBERL(aIndex);
	pMsg.MemberID[MAX_ACCOUNT_LEN] = 0;	
	pMsg.GuildName[MAX_GUILD_LEN] = 0;
		...
		if ( gObj[n].Connected > PLAYER_LOGGED )
		{
			if ( gObj[n].Name[0] == lpMsg->MemberID[0] )
			{
				if ( strcmp(lpMsg->MemberID, gObj[n].Name ) == 0 )
				{
					...
					/*gObj[n].m_ViewSkillState &= 0xFFF3FFFF;
					gObj[n].m_ViewSkillState &= 0xFFBFFFFF;
					gObj[n].m_ViewSkillState &= 0xFF7FFFFF;*/
					...
					if ( gObj[n].lpGuild != NULL )
					{
						...
						{
							g_ArcaBattle.PrintGuildMemberLog(gObj[n].lpGuild);
						}
#endif
					}
					else
					{
#if (GS_CASTLE == 1)
						gObj[n].m_bCsGuildInvolved = FALSE;
...
void GDGuildNoticeSave(LPSTR guild_name, LPSTR guild_notice)
{
	...
	for(int i = 0; i < len;i++)
	{
		if(guild_notice[i] == '\'' || guild_notice[i] == '"')
			return;
	}
...
void DGGuildNoticeRecv(SDHP_GUILDNOTICE * lpMsg)
{
	...
	memset(lpNode->Notice, 0, sizeof(lpNode->Notice));
	strncpy(lpNode->Notice, lpMsg->szGuildNotice,(sizeof(lpNode->Notice)-1));
