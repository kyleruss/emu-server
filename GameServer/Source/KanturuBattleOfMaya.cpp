...
CKanturuBattleOfMaya::CKanturuBattleOfMaya()
{
	...
	int iCount;
	for ( iCount=0;iCount<KANTURU_MAYA_AI_CHANGE_TIME;iCount++)
	{
...
void CKanturuBattleOfMaya::ResetAllData()
{
	...
	int iCount;
	for ( iCount=0;iCount<KANTURU_MAYA_TIME_INFO;iCount++)
	{
...
{
	if ( (GetTickCount() - this->m_iMayaHandAIAppliedTime) >= (DWORD)this->m_iMayaHandAIChangeTime[this->m_iMayaHandCurrentAI] )
	{
		this->m_iMayaHandAIAppliedTime = GetTickCount();
