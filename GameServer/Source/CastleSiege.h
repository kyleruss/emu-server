...
struct _CS_REG_GUILD_DATA
{
	...
	int m_bIN_USE;
	basic_string<char,char_traits<char> > m_strGuildName;
	int m_iRegMarkCount;
...
public:
	...
	void SetDbDataLoadOK(int bDbDataLoadOK);
	...
	int GetCastleState();
	_SYSTEMTIME GetCastleLeftSiegeDate();
	int CalcCastleLeftSiegeDate();
	int GetCastleStateTerm(_SYSTEMTIME * tmStateStartDate,_SYSTEMTIME * tmStateEndDate);
	int GetCurRemainSec();
	void ClearDbNPC();
	void ClearNonDbNPC();
	...
	void SetCrownIndex(int iIndex);
	void NotifyCrownState(BYTE btState);
	int GetRegCrownAvailable();
	void SetRegCrownAvailable(int bRegCrownAvailable);
	int GetCastleTowerAccessable();
	void SetCastleTowerAccessable(int bTowerAccessable);
	...
	void SetIsSiegeGuildList(int bIsSiegGuildList);
	void SendCastleStateSync();
	int GetDbNpcCreated();
	void SetDbNpcCreated(int bDbNpcCreated);
	...
	int GetHuntZoneEnter();
	void SetHuntZoneEnter(int bEnterHuntZone);
	...
	void ResetCrownUserIndex();
	int GetCrownAccessUserX();
	int GetCrownAccessUserY();
	void SetCrownAccessUserX(BYTE iX);
	void SetCrownAccessUserY(BYTE iY);
...
/*this+0x30*/ _SYSTEMTIME m_tmFixCastleCycleStartDate;
/*this+0x40*/ int m_iFixCastleSpecificState;
/*this+0x44*/ int m_bFixCastleStateStartDate;
/*this+0x48*/ _SYSTEMTIME m_tmFixCastleStateStartDate;
/*this+0x58*/ int m_iCS_REMAIN_MSEC;
/*this+0x5C*/ int m_iCS_TICK_COUNT;
/*this+0x60*/ _SYSTEMTIME m_tmStateStartDate;
/*this+0x70*/ _SYSTEMTIME m_tmLeftCastleSiegeDate;
...
/*this+0xA4*/ _SYSTEMTIME m_tmStartDate;
/*this+0xB4*/ _SYSTEMTIME m_tmEndDate;
...
/*this+0x100*/ _RTL_CRITICAL_SECTION m_critScheduleData;
/*this+0x118*/ vector<_CS_NPC_DATA> m_vtNpcData;
/*this+0x128*/ _RTL_CRITICAL_SECTION m_critNpcData;
/*this+0x140*/ int m_bDbNpcCreated;
/*this+0x144*/ map<basic_string<char,char_traits<char> >,_CS_TOTAL_GUILD_DATA,less<basic_string<char,char_traits<char> > > > m_mapCsBasicGuildInfo;
/*this+0x154*/ _RTL_CRITICAL_SECTION m_critCsBasicGuildInfo;
/*this+0x16C*/ map<basic_string<char,char_traits<char> >,_CS_TOTAL_GUILD_DATA,less<basic_string<char,char_traits<char> > > > m_mapCsTotalGuildInfo;
/*this+0x17C*/ _RTL_CRITICAL_SECTION m_critCsTotalGuildInfo;
...
/*this+0x1AC*/ _RTL_CRITICAL_SECTION m_critCsMiniMap;
...
extern int g_iNpcUpDfLevel_CGATE[CS_MAX_UPGRADE][0x3];
extern int g_iNpcUpDfLevel_CSTATUE[CS_MAX_UPGRADE][0x3];
extern int g_iNpcUpMaxHP_CGATE[0x3][CS_MAX_UPGRADE];
extern int g_iNpcUpMaxHP_CSTATUE[CS_MAX_UPGRADE][0x3];
extern int g_iNpcUpRgLevel_CSTATUE[CS_MAX_UPGRADE][0x3];
extern int g_iCsUserReSpawnArea[0x3][0x4];
extern int g_iNpcDefense_CGATE[CS_MAX_UPGRADE + 1];
extern int g_iNpcDefense_CSTATUE[CS_MAX_UPGRADE + 1];
extern int g_iCastleWallStepAxisY[3];
