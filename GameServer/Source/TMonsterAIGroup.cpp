		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					Token = GetToken();
					iClass = (int)TokenNumber;

					Token = GetToken();
					iRank = (int)TokenNumber;

					Token = GetToken();
					iStartAI = (int)TokenNumber;

					Token = GetToken();
					iAI01 = (int)TokenNumber;

					Token = GetToken();
					iAI02 = (int)TokenNumber;

					Token = GetToken();
					iAI03 = (int)TokenNumber;

					Token = GetToken();
					iCreateType = (int)TokenNumber;

					Token = GetToken();
					iMapNumber = (int)TokenNumber;

					Token = GetToken();
					iStartX = (int)TokenNumber;

					Token = GetToken();
					iStartY = (int)TokenNumber;

					Token = GetToken();
					iStartDir = (int)TokenNumber;

					Token = GetToken();
					iRegenType = (int)TokenNumber;
...
TMonsterAIGroupMember * TMonsterAIGroup::FindGroupMemberToHeal(int iObjIndex, int iGroupNumber, int iGuid, int iDistance)
{
	...
	for ( int i=0;i<iCount;i++)
	{
		if ( TMonsterAIGroup::s_MonsterAIGroupMemberArray[iGroupNumber][i].m_iGroupNumber == iGroupNumber )
		{
			...
			int iMaxLife = (int)(lpTargetObj->MaxLife + 1.0f);
			int iPercentHP = (int)(lpTargetObj->Life * 100.0f / iMaxLife);
