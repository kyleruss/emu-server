...
BOOL CNewPVP::DropItem(LPOBJ lpObj,BYTE btMapNumber,BYTE cX, BYTE cY, int nMaxHitUser)	//OK
{
	...
	g_EventItemBagManager.OpenSpecial(EventBagSpecial::DuelArena, lpObj->m_Index, btMapNumber, cX, cY);
	return TRUE;
}
...
int OnCGInviteDuel(LPPMSG_REQ_DUEL_INVITE lpMsg, int aIndex)
{
	...
	LPOBJ lpRequester = (LPOBJ)&gObj[aIndex];
	...
	if( !gObjIsConnected(nResponsor) )
	{
		LogAdd("error : %s %d (%d)", __FILE__, __LINE__, nResponsor);
	}
	LPOBJ lpResponsor = (LPOBJ)&gObj[nResponsor];
...
{
	LPOBJ lpRequester = NULL;
	LPOBJ lpResponsor = (LPOBJ)&gObj[aIndex];
	int nRequester = MAKEWORD(lpMsg->NumberL, lpMsg->NumberH);

	if( !gObjIsConnected(nRequester) )	return ENEWPVP::E_DISCONNECT;
	lpRequester = (LPOBJ)&gObj[nRequester];
