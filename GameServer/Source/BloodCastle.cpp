...
int g_iBC_ChoasMixSuccessRate[MAX_BLOOD_CASTLE_LEVEL] = { 80, 80, 80, 80, 80, 80, 80, 80 };
int g_iBC_ChoasMixMoney[MAX_BLOOD_CASTLE_LEVEL] = {50000 , 80000 , 150000, 250000, 400000, 600000, 850000, 1000000};
...
CBloodCastle::CBloodCastle()
{
	...
	for ( int i =0; i<MAX_BLOOD_CASTLE_LEVEL ; i++ )
	{
		...
		this->m_BridgeData[i].m_iMapNumber = GetEventMap(i);
		...
		this->m_BridgeData[i].m_iBC_REWARD_MULTIPLY = 1.0f;
		this->m_BridgeData[i].m_iBC_CASTLE_BLOCKINFO = 1000;
...
void CBloodCastle::Init(bool bEVENT_ENABLE)
{
	...
	{
		for ( n=0;n<MAX_BLOOD_CASTLE_BOSS_MONSTER;n++)
		{
			this->m_BridgeData[i].m_iAngelKingPosNum = -1;
	...
	for ( n =0; n<OBJ_MAXMONSTER;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber ) != FALSE )
		{
			...
			BYTE btBloodCastleIndex = GetBridgeLevel(btMapNumber);
	...
	while ( true )
	{
		...
		while ( true )
		{
			if ( type == 0 )
			{
				...
				this->m_iBC_TIME_MIN_OPEN = (int)TokenNumber;

				Token = GetToken();
				this->m_iBC_TIME_MIN_PLAY =(int)TokenNumber;

				Token = GetToken();
				this->m_iBC_TIME_MIN_REST =(int)TokenNumber;
			}
			else if ( type == 1 )
			{
				Token = GetToken();
				...
				this->m_iBC_NORMAL_ITEM_DROP = (int)TokenNumber;

				Token = GetToken();
				this->m_iBC_EXCEL_ITEM_DROP = (int)TokenNumber;

				Token = GetToken();
				this->m_iBC_MONSTER_REGEN = (int)TokenNumber;
			}
			else if ( type == 2 )
			{
				Token = GetToken();
			...
			else if ( type == 3 )
			{
				...
				{
					Token = GetToken();
					this->m_BridgeData[iBridgeNum].m_iBC_REWARD_MULTIPLY = TokenNumber;
				}
				else
				{
					Token = GetToken();
			...
			else if ( type == 4 )
			{
				...
				pTmp.Hour = (int)TokenNumber;
				Token = GetToken();
				pTmp.Min = (int)TokenNumber;

				this->m_EventTime.push_back(pTmp);
			}
		}
	}
...
{
	g_bBloodCastle=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "BloodCastleEvent", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	g_iAngelKingsPaperDropRate=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "AngelKingsPaperDropRate", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	g_iBloodBoneDropRate=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "BloodBoneDropRate", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	g_iStoneDropRate=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "StoneDropRate", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
	g_bStoneItemDrop=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "StoneItemDrop", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
}
...
void CBloodCastle::CheckSync(int iBridgeIndex)
{
	...
	std::list<_EVENT_TIME>::iterator Itor_;

	_EVENT_TIME pFirstTime;
	_EVENT_TIME pTime;
	...
	while( CHECK_COUNT-- )
	{
		...
		if( today->tm_hour <= pTime.Hour &&
			CHECK_TIME > NOW_TIME )
		{
			this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC = (CHECK_TIME -NOW_TIME)*60*1000;
		}
		else
		{
			this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC = (1440 - NOW_TIME + CHECK_TIME)*60*1000;
		}

		if( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= m_iBC_TIME_MIN_OPEN*60*1000 )
		{
			Itor_++;
			if( Itor_ != m_EventTime.end() )
			{
	...
	LogAddTD("[BloodCastle] (%d) Sync Open Time. [%d] min remain",iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC/60000);
...
void CBloodCastle::ClearBridgeData(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		...
		this->m_BridgeData[iBridgeIndex].m_iBC_DOOR_SUBSIST_INDEX = -1;
		this->m_BridgeData[iBridgeIndex].m_iBC_CASTLE_BLOCKINFO = 1000;
	}
}
...
void CBloodCastle::ProcState_Playing(int iBridgeIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
		...
		if ( this->m_BridgeData[iBridgeIndex].m_bBC_MONSTER_KILL_COMPLETE != false	)
		{
			if ( this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN != -1 )
			{
				if ( GetTickCount() > this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN )
				{
					...
					this->m_BridgeData[iBridgeIndex].m_iBC_CASTLE_BLOCKINFO = 1002;

					if( this->m_BridgeData[iBridgeIndex].m_iBC_DOOR_SUBSIST_INDEX == -1 )
					{
					...
					else
					{
						LogAddTD("[Blood Castle] (%d) Door Subsist (%d)(Name: %s)(Dieregen:%d)",
							iBridgeIndex+1,m_BridgeData[iBridgeIndex].m_iBC_DOOR_SUBSIST_INDEX,gObj[m_BridgeData[iBridgeIndex].m_iBC_DOOR_SUBSIST_INDEX].Name,gObj[m_BridgeData[iBridgeIndex].m_iBC_DOOR_SUBSIST_INDEX].DieRegen);
					}
				}
			}
		}
		...
		if ( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= ((this->m_iBC_TIME_MIN_PLAY*60-60)*1000) && this->m_BridgeData[iBridgeIndex].m_bBC_PLAY_START == false )
		{
			...
			this->m_BridgeData[iBridgeIndex].m_iBC_CASTLE_BLOCKINFO = 1001;
...
void CBloodCastle::SetState_None(int iBridgeIndex)
{
	...
	{
		int MapNumber = GetEventMap(iBridgeIndex);
		if ( gObj[n].MapNumber == MapNumber && gObj[n].Connected == PLAYER_PLAYING )
		{
			gObjMoveGate(n, 22);
		}
	}
...
void CBloodCastle::SetState_Closed(int iBridgeIndex)
{
	...
	{
		int MapNumber = GetEventMap(iBridgeIndex);
		if ( gObj[n].MapNumber == MapNumber && gObj[n].Connected > PLAYER_LOGGED )
		{
...
void CBloodCastle::SetState_Playing(int iBridgeIndex)
{
	...
	for (int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].MapNumber == GetEventMap(iBridgeIndex) && gObj[n].Connected > PLAYER_LOGGED )
		{
			this->SearchUserDeleteQuestItem(n);
		}
	}
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	int MapNumber = GetEventMap(iLEVEL-1);

	if ( BC_MAP_RANGE( (MapNumber) )  == FALSE )
	{
		return false;
	}
	...
	LogAddTD("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Start (Account:%s, Name:%s, Level:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);

	char szTmp[20];

	wsprintf(szTmp,"BloodCastle Mix %d",iLEVEL);
	...
	if ( iMIX_SUCCESS_RATE < 0 || iMIX_SUCCESS_RATE > 100 )
	{
		...
		LogAddTD("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - MixRate Out of Bound (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	{
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAddTD("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - MixMoney < 0 (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ( (gObj[iIndex].Money - iMIX_NEED_MONEY) < 0 )	// Not enoght zen
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAddTD("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - Not Enough Money (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ( (rand()%100) < iMIX_SUCCESS_RATE )	// Siccess
	{
		...
		LogAddTD("[BloodCastle] [%s][%s] CBMix Success %d Money : %d-%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
#if( __4GAMERS__ == 1 )
	...
	else	// Failure
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAddTD("[BloodCastle] [%s][%s] CBMix Fail %d Money : %d-%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
...
static const int g_iBC_EventScore_Fail[MAX_BLOOD_CASTLE_LEVEL] = { -300,-300,-300,-300,-300,-300,-300 };	
...
int  CBloodCastle::CheckQuestItem(int iIndex)
{
	...
	int BridgeIndex = GetBridgeLevel(gObj[iIndex].MapNumber);
...
bool CBloodCastle::CheckWalk(int iIndex, int iMoveX, int iMoveY)
{
	...
	if ( this->GetCurrentState(GetBridgeLevel(gObj[iIndex].MapNumber)) == TRUE )
	{
...
int  CBloodCastle::GetRemainTime(int iBridgeIndex)
{
	...
	if ( GetCurrentState(iBridgeIndex) == 1 )
	{
		iREMAIN_MINUTE = m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC / 60000 - m_iBC_TIME_MIN_OPEN + 1;
	}
	else
	{
		std::list<_EVENT_TIME>::iterator Itor_;
		...
		while( CHECK_COUNT-- )
		{
			...
			if( today->tm_hour <= pTime.Hour &&
				CHECK_TIME > NOW_TIME )
			{
				iREMAIN_MINUTE = (CHECK_TIME -NOW_TIME)*60*1000;
			}
			else
			{
				iREMAIN_MINUTE = (1440 - NOW_TIME + CHECK_TIME)*60*1000;
			}
...
void CBloodCastle::ClearMonster(int iBridgeIndex, bool bClearCastleDoor)
{
	...
	{
		if ( gObj[n].MapNumber == GetEventMap(iBridgeIndex) )
		{
...
void CBloodCastle::SetMonster(int iBridgeIndex)
{
	...
	if ( nCastleDoorPosNum != -1 )
	{
		if ( btMonsterType == 131 )
		{
			...
			if ( iIndex >= 0 )
			{
				...
				this->m_BridgeData[btBloodCastleIndex].m_iBC_DOOR_SUBSIST_INDEX = iIndex;
				LogAddTD("[Blood Castle] (%d) Create Door Successfuly : Index(%d)",btBloodCastleIndex+1,iIndex);
			}
			else
			{
	...
	for ( int n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE  )
		{
			...
			BYTE btBridgeIndex = GetBridgeLevel(btMap);
	...
	if ( nSaintStatuePosNum != -1 )
	{
		...
		else
		{
			LogAddTD("[Blood Castle][Bug Tracer] (%d) It's not Saint Statue( Map:%d, BloodCastleIndex:%d, Type:%d )",
				iBridgeIndex+1, btMapNumber, GetBridgeLevel(btMapNumber), btMonsterType);
		}
	}
...
int  CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		else if( gObj[iIndex].Class == CLASS_MAGUMSA )
		{
			gObj[iIndex].LevelUpPoint += 7;
		}
		else if( gObj[iIndex].Class == CLASS_FIGHTER )
		{
			gObj[iIndex].LevelUpPoint += 7;
		}
		else
		...
		{
			int PartyNumber = gObj[iIndex].PartyNumber;
			char szLog[256];

			sprintf(szLog,"400 LevelUp (%s)(%s) Party ",gObj[iIndex].AccountID,gObj[iIndex].Name);
			...
			for(int i = 0; i < MAX_USER_IN_PARTY; i++)
			{
				...
				if( tObjNum >= 0 )
				{
					int Len = strlen(szLog);
					sprintf(&szLog[Len],",(%s)(%s) ",gObj[tObjNum].AccountID,gObj[tObjNum].Name);
				}
			}
...
void CBloodCastle::CheckUsersOnConnect(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex != -1 )
		{
			...
			{
				if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber != GetEventMap(iBridgeIndex) )
				{
					this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex = -1;
				}
			}
...
void CBloodCastle::BlockCastleEntrance(int iBridgeIndex)
{
	...
	for ( int i= ::g_btCastleEntranceMapXY[iBridgeIndex].btStartX;i<= ::g_btCastleEntranceMapXY[iBridgeIndex].btEndX;i++)
	{
		...
		{
			int MapNumber = GetEventMap(iBridgeIndex);
			MapC[MapNumber].m_attrbuf[j * 256 + i] |= 4;
		}
	}
}
...
void CBloodCastle::BlockCastleBridge(int iBridgeIndex)
{
	...
	for ( int i= ::g_btCastleBridgeMapXY[iBridgeIndex].btStartX;i<= ::g_btCastleBridgeMapXY[iBridgeIndex].btEndX;i++)
	{
		...
		{
			int MapNumber = GetEventMap(iBridgeIndex);
			MapC[MapNumber].m_attrbuf[j * 256 + i] |= 8;
		}
	}
}
...
void CBloodCastle::BlockCastleDoor(int iBridgeIndex)
{
	...
	for ( int x=0;x<3;x++)
	{
		for ( int y=::g_btCastleDoorMapXY[iBridgeIndex][x].btStartX; y <= ::g_btCastleDoorMapXY[iBridgeIndex][x].btEndX ;y++)
		{
			...
			{
				int MapNumber = GetEventMap(iBridgeIndex);
				MapC[MapNumber].m_attrbuf[z * 256 + y] |= 4;
			}
		}
	}
}
		...
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19) )
			{
				...
				if ( BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE )
				{
					...
					int iBridgeIndex = GetBridgeLevel(gObj[iIndex].MapNumber);

					this->DropItemDirectly(iBridgeIndex,gObj[iIndex].m_Index, ITEMGET(13, 19), x);
					this->DropItemDirectly(gObj[iIndex].MapNumber - MAP_INDEX_BLOODCASTLE1,gObj[iIndex].m_Index, ITEMGET(13, 19), x);

					if ( BC_MAP_RANGE(gObj[iIndex].MapNumber) != FALSE )
					{
						int iBC_INDEX = GetBridgeLevel(gObj[iIndex].MapNumber);
...
#ifdef POINTEX
			g_ShopPointEx.AddEventBonus(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, ShopPointExEvent::BC);
#endif

#if( __4GAMERS__ == 1 )	
			g_Achievements.GD_UpdateEventDataBC(&gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex], 
				gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_iBloodCastleEXP,
				iREWARD_EXP, iREWARD_SCR);
#endif
		}
...
int  CBloodCastle::CalcSendRewardEXP(int iIndex, int iEXP)
{
	...
	if ( gObj[iIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			if ( iCAL_EXP > 0 )
			{
				CheckItemOptForGetExpEx(&gObj[iIndex],iCAL_EXP,1);
				iCAL_EXP = __int64(iCAL_EXP*m_BridgeData[gObj[iIndex].m_cBloodCastleIndex].m_iBC_REWARD_MULTIPLY);
				iRET_EXP = int(iCAL_EXP);
				iCAL_EXP = this->LevelUp(iIndex, (int)iCAL_EXP, 4);
			}
		}
...
void CBloodCastle::DropReward(int iIndex)
{
	...
	{
	case 11:	DropIndex = 238;	break;
	case 12:	DropIndex = 239;	break;
	case 13:	DropIndex = 240;	break;
	case 14:	DropIndex = 241;	break;
	case 15:	DropIndex = 242;	break;
	case 16:	DropIndex = 243;	break;
	case 17:	DropIndex = 244;	break;
	case 52:	DropIndex = 245;	break;
	}

	switch( gObj[iIndex].MapNumber )
	{
	case 11:
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle1, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 12:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle2, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 13:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle3, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 14:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle4, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 15:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle5, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 16:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle6, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 17:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle7, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	case 52:	
		g_EventItemBagManager.OpenSpecial(EventBagSpecial::BloodCastle8, iIndex, DropIndex, gObj[iIndex].X, gObj[iIndex].Y);
		break;
	}
}
...
bool CBloodCastle::CheckAngelKingExist(int iBridgeIndex)
{
	...
	for ( int i=0;i<OBJ_STARTUSERINDEX;i++)
	{
		if ( gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_NPC )
		{
			if ( gObj[i].Class == 232 )
			{
				if (gObj[i].MapNumber == GetEventMap(iBridgeIndex) )
				{
					bRET_VAL = true;
					break;
				}
	...
	if ( bRET_VAL == false )
	{
		int result = gObjAddMonster(GetEventMap(iBridgeIndex));

		if ( result >= 0 )
		{
			gObjSetPosMonster(result, this->m_BridgeData[iBridgeIndex].m_iAngelKingPosNum);
			gObjSetMonster(result, 232);
			gObj[result].m_cBloodCastleIndex = GetBridgeLevel(gObj[result].MapNumber);
			bRET_VAL = true;
		}
		else
		{
...
{
	if ( BC_MAP_RANGE(GetEventMap(iBridgeIndex)) == FALSE )
	{
	...
	if ( MapC[map_num].ItemDrop(type, level, dur, gObj[iIndex].X, gObj[iIndex].Y,Option1, Option2, Option3, NOption, SOption, item_number, aAntiLootIndex, PetLevel, PetExp, ItemEffectEx, SocketOption,SocketBonus) == TRUE )
	{
...
int CBloodCastle::GetEventMap(int iBridgeIndex)
{
	int iMap = 0;

	switch( iBridgeIndex )
	{
	case 0:	iMap = 11;	break;
	case 1:	iMap = 12;	break;
	case 2:	iMap = 13;	break;
	case 3:	iMap = 14;	break;
	case 4:	iMap = 15;	break;
	case 5:	iMap = 16;	break;
	case 6:	iMap = 17;	break;
	case 7:	iMap = 52;	break;
	}

return iMap;
}

int CBloodCastle::GetBridgeLevel(int iMapNumber)
{
	int iLevel = -1;

	switch( iMapNumber )
	{
	case 11:	iLevel = 0; break;
	case 12:	iLevel = 1; break;
	case 13:	iLevel = 2; break;
	case 14:	iLevel = 3; break;
	case 15:	iLevel = 4; break;
	case 16:	iLevel = 5; break;
	case 17:	iLevel = 6; break;
	case 52:	iLevel = 7; break;
	}
return iLevel;
}

int CBloodCastle::GetItemMap(int iRetMap)
{
	int MapNumber = iRetMap;

	switch( iRetMap )
	{
	case 238:	MapNumber = 11;	break;
	case 239:	MapNumber = 12;	break;
	case 240:	MapNumber = 13;	break;
	case 241:	MapNumber = 14;	break;
	case 242:	MapNumber = 15;	break;
	case 243:	MapNumber = 16;	break;
	case 244:	MapNumber = 17;	break;
	case 245:	MapNumber = 52;	break;
	}
return MapNumber;
}

int CBloodCastle::GetRewardMap(int iRetMap)
{
	int MapNumber = iRetMap;

	switch( iRetMap )
	{
	case 246:	MapNumber = 11;	break;
	case 247:	MapNumber = 12;	break;
	case 248:	MapNumber = 13;	break;
	case 249:	MapNumber = 14;	break;
	case 250:	MapNumber = 15;	break;
	case 251:	MapNumber = 16;	break;
	case 252:	MapNumber = 17;	break;
	case 253:	MapNumber = 52;	break;
	}
return MapNumber;
}

int CBloodCastle::ChangeUserIndex(int iExUserIndex, int iCurrentUserIndex, int iBridgeIndex)
{
	if( !BC_BRIDGE_RANGE(iBridgeIndex) )
		return false;
...
void CBloodCastle::SendNoticeMessageToSpecificUser(int iBridgeIndex,int iUserIndex,int iPlayState)
{
	PMSG_STATEBLOODCASTLE pMsg;
	PHeadSetB((LPBYTE)&pMsg, 0x9B, sizeof(PMSG_STATEBLOODCASTLE));
...
int CBloodCastle::SetCastleBlockInfo(int iBridgeIndex,int iCastleBlockInfo)
{
	if( !BC_BRIDGE_RANGE(iBridgeIndex) )
		return false;

	if( !BC_BLOCKINFO(iCastleBlockInfo) )
		return false;

	this->m_BridgeData[iBridgeIndex].m_iBC_CASTLE_BLOCKINFO = iCastleBlockInfo;
	return true;
}
...
int CBloodCastle::GetCastleBlockInfo(int iBridgeIndex)
{
	...
	return this->m_BridgeData[iBridgeIndex].m_iBC_CASTLE_BLOCKINFO;
}
