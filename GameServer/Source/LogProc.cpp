...
void LogDataSet()
{
	...
	wsprintf(szTemp,"%s%02d%02d%02d_%s.log", LOG_PATH_MAIN, LogMYear, LogMonth, LogMDay,&szServerName);
...
void LogAddHeadHexFunc(int Type,char* data,int len)
{
	...
	char* szTmp = new char[len*4+1];
	memset(szTmp,0,len*4+1);
	...
	else
	{
		...
		else
		{
			...
			{
				char tmpstr[8];
				sprintf(tmpstr,"%02X,",(BYTE)data[n]&0xFF);
				strcat(szTmp,tmpstr);
				fprintf(logfp,"%02X,",(BYTE)data[n]&0xFF);
			}

			LogTextAdd(2,szTmp,strlen(szTmp));
