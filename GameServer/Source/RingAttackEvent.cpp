	...
	while ( true )
	{
		...
		while ( true )
		{
			if ( type == 0 )
			{
				...
				this->m_iTIME_MIN_OPEN = (int)TokenNumber;

				Token = GetToken();
				this->m_iTIME_MIN_PLAY = (int)TokenNumber;

				Token = GetToken();
				this->m_iTIME_MONSTER_TO_DEST = (int)TokenNumber;
			}
			else if ( type == 1 )
			{
				Token = GetToken();
			...
			else if ( type == 2 )
			{
				...
				pRMD.m_iType = (int)TokenNumber;

				Token = GetToken();
				pRMD.m_bDoRegen = (int)TokenNumber;

				Token= GetToken();
				pRMD.m_bDoAttackFirst = (int)TokenNumber;
