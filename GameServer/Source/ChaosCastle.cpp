...
CChaosCastle::CChaosCastle()
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_LEVEL;i++)
	{
		...
		this->m_stChaosCastleData[i].m_iMapNumber = GetMap(i);
		...
		this->m_stChaosCastleData[i].m_iCC_REWARD_MULTIPLY = 1.0f;
	...
	while ( true )
	{
		...
		while ( true )
		{
			if ( type == 0 )	// Time Setting
			{
				...
				this->m_iCC_TIME_MIN_OPEN = (int)TokenNumber;

				Token = GetToken();
				this->m_iCC_TIME_MIN_PLAY = (int)TokenNumber;

				Token = GetToken();
				this->m_iCC_TIME_MIN_REST = (int)TokenNumber;
			}
			else if ( type == 1 ) // Drops
			{
				...
				this->m_iCC_NORMAL_ITEM_DROP = (int)TokenNumber;

				Token = GetToken();
				this->m_iCC_EXCEL_ITEM_DROP = (int)TokenNumber;

				Token = GetToken();
				this->m_iCC_MONSTER_REGEN = (int)TokenNumber;
			}
			else if ( type == 2 ) // Schedule
			{
				...
				Schedule.m_iHour = (int)TokenNumber;

				Token = GetToken();
				Schedule.m_iMinute = (int)TokenNumber;
			...
			else if ( type == 4 )
			{
				...
				int iFloor = (int)TokenNumber;

				if( CC_FLOOR_RANGE(iFloor) )
				{
					Token = GetToken();
					m_stChaosCastleData[iFloor].m_iCC_REWARD_MULTIPLY = TokenNumber;
				}
				else
				{
					Token = GetToken();
...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	std::vector<CHAOSCASTLE_START_TIME>::iterator it = this->m_vtChaosCastleOpenTime.begin();
...
void CChaosCastle::ProcState_Playing(int iChaosCastleIndex)
{
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 0 )
	{
		...
		else if ( this->GetCurEnteredUser(iChaosCastleIndex) == 1 )
		{
			for( int i =0; i < MAX_CHAOSCASTLE_MONSTER; i++)
			{
				...
				int iMonIndex = this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[i];
	
				if ( OBJMON_RANGE(iMonIndex) == FALSE )
				{
					this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[i] = -1;
					continue;
				}	
	
				if ( gObj[iMonIndex].Life > 0 &&
						CC_MAP_RANGE(gObj[iMonIndex].MapNumber) &&
					gObj[iMonIndex].Connected > 0)
				{
					LogAddTD("[Chaos Castle][Bug Tracer] (%d) Left Monster AttrInfo %d/%d",
						iChaosCastleIndex+1,gObj[iMonIndex].X,gObj[iMonIndex].Y);
				}	
			}
		}
		else
...
void CChaosCastle::PullObjInnerPlace(int iChaosCastleIndex, int iTRAP_STEP)
{
	...
	for ( int iAXIS = 0; iAXIS < MAX_CC_TRAP_STEP ; iAXIS++)
	{
		for ( int iMAPX = g_rtPOINT_TRAP[iAXIS].left ; iMAPX <= g_rtPOINT_TRAP[iAXIS].right ; iMAPX ++ )
		{
			...
			{
				int MapNumber = GetMap(iChaosCastleIndex);
				BYTE btMapAttr = MapC[MapNumber].GetAttr(iMAPX, iMAPY);
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		...
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE && gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == GetMap(iChaosCastleIndex) )
			{
				...
				BYTE btMapAttr = MapC[MapNumber].m_attrbuf[iSY * 256 + iSX] & 0x08;
	...
	for ( int iMAP_ITEM =0;iMAP_ITEM<MAX_MAPITEM;iMAP_ITEM++)
	{
		...
		if ( MapC[MapNumber].m_cItem[iMAP_ITEM].IsItem() )
		{

			int x = MapC[MapNumber].m_cItem[iMAP_ITEM].px;
			int y = MapC[MapNumber].m_cItem[iMAP_ITEM].py;
			...
			{
				MapC[MapNumber].m_cItem[iMAP_ITEM].m_State = 8;
				MapC[MapNumber].m_cItem[iMAP_ITEM].Give = true;
				MapC[MapNumber].m_cItem[iMAP_ITEM].live = false;
			}
		}
	}
}
...
void CChaosCastle::SetMapAttrHollow(int iChaosCastleIndex, int iTRAP_STEP)
{
	...
	for ( int a=0;a<MAX_CC_HOLLOW_ZONE;a++)
	{
		for ( int b=::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][0] ; b<=::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][2] ; b++)
		{
			...
			{
				int MapNumber = GetMap(iChaosCastleIndex);
				MapC[MapNumber].m_attrbuf[ c * 256 + b] |= 8;
			}
		}
	}
...
void CChaosCastle::SetState_Closed(int iChaosCastleIndex)
{
	...
	{
		if ( gObj[n].MapNumber == GetMap(iChaosCastleIndex) )
		{
...
int  CChaosCastle::SetMonster(int iChaosCastleIndex)
{
	...
	for ( int n=0;n<OBJ_MAXMONSTER;n++)
	{
		...
		if ( CC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE )
		{
			...
			BYTE btChaosCastleIndex = GetFloor(btMapNumber);
...
void CChaosCastle::ProcessFallUser(int iChaosCastleIndex)
{
	...
	for ( int iFALL=0;iFALL<MAX_CHAOSCASTLE_USER;iFALL++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_lFallUser[iFALL] != -1 )
		{
			...
			if ( (gObj[iFALL_INDEX].MapNumber) == GetMap(iChaosCastleIndex) &&
	...
	if ( gObj[iUserIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			if ( iCAL_EXP > 0 )
			{
				...
				iCAL_EXP = __int64(iCAL_EXP*this->m_stChaosCastleData[gObj[iUserIndex].m_cChaosCastleIndex].m_iCC_REWARD_MULTIPLY);
				iRET_EXP = int(iCAL_EXP);
				iCAL_EXP = this->LevelUp(iUserIndex, (int)iCAL_EXP, 3);
			}
		...
		{
			GCSendExp(iUserIndex,(WORD)-1,iRET_EXP,0,0);
		}
	}
...
#if (__CUSTOM__ == 1 )
	if ( gObj[iUserIndex].ChangeUP2 == TRUE )
#else
	...
	else
	{
		for(int i=0;i<MAX_CHAOSCASTLE_LEVEL;i++)
		{
			if ( gObj[iUserIndex].Class == CLASS_DARKLORD ||
				...
				gObj[iUserIndex].Class == CLASS_FIGHTER )
			{
...
#if (CUSTOM_DAMAGE == 1)
	pResult.DamageHW = SET_NUMBERHW(iDamage);
	pResult.DamageLW = SET_NUMBERLW(iDamage);

	pResult.wShieldDamageHW = 0;
	pResult.wShieldDamageLW = 0;
#else
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		if( gObj[iUserIndex].Level == 400 && gObj[iUserIndex].PartyNumber >= 0)
		{
			...
			sprintf(szLog,"400 LevelUp (%s)(%s) Party ",gObj[iUserIndex].AccountID,gObj[iUserIndex].Name);
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == GetMap(iChaosCastleIndex) &&	
				this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iUserState == 0 )
			{
...
void CChaosCastle::SearchNBlowObjs(int iMapNumber, int iX, int iY)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		int iCastleIndex = GetFloor(iMapNumber);
		if ( this->m_stChaosCastleData[iCastleIndex].m_UserData[i].m_iIndex == -1 )
			continue;

		int iIndex = this->m_stChaosCastleData[iCastleIndex].m_UserData[i].m_iIndex;
		...
		if ( gObj[iIndex].MapNumber == iMapNumber && gObj[iIndex].Connected > PLAYER_LOGGED )
		{
				 ...
				 gObj[iIndex].Y >= iMIN_Y && gObj[iIndex].Y <= iMAX_Y )
			{
				...
				if ( btMapAttr == 8 )
				{
					this->AddFallUser(iCastleIndex, iIndex);

					LogAddTD("[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
						iCastleIndex+1, gObj[iIndex].AccountID, gObj[iIndex].Name, iX, iY);
				}
			}
		}
	}
...
void CChaosCastle::SafetyCastleZone(int iChaosCastleIndex)
{
	...
	for ( int i=g_btCastleSafetyZoneMapXY[iChaosCastleIndex][0] ; i <= g_btCastleSafetyZoneMapXY[iChaosCastleIndex][2] ; i++ )
	{
		...
		{
			MapC[MapNumber].m_attrbuf[ j * 256 + i] |= 1;
		}
	}
...
void CChaosCastle::SendCastleZoneSafetyInfo(int iChaosCastleIndex, bool bDoSet)
{
	...
	for (int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == GetMap(iChaosCastleIndex) )
		{
...
#if( __4GAMERS__ == 1 )
			g_Achievements.GD_UpdateEventDataCC(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex], 
				iKILLCOUNT_USER, iKILLCOUNT_MONSTER, iTOT_EXP, iREWARD_EXP);
#endif
		}
	}
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == GetMap(iChaosCastleIndex) )
			{
					   ...
					   iSY > g_iChaosCastle_DamageAxis[iTRAP_STEP][3] ) )
				{
					...
					if ( gObjCheckUsedBuffEffect(lpObj,BUFF_ICE) == FALSE )
					{
						lpObj->DelayActionTime = 800;
						lpObj->DelayLevel = 1;
						lpObj->lpAttackObj = NULL;

						gObjAddBuffEffect(lpObj,BUFF_ICE,0,0,0,0,3);
					}
				}
			}
		}
...
void CChaosCastle::GiveWinnerItem(int iChaosCastleIndex, int iWinnerIndex)
{
	...
	BYTE SpecialType = iChaosCastleIndex + EventBagSpecial::ChaosCastle1;

	if( SpecialType < EventBagSpecial::ChaosCastle1 || SpecialType > EventBagSpecial::ChaosCastle7 )
	{
		SpecialType = EventBagSpecial::ChaosCastle1;
	}

	g_EventItemBagManager.OpenSpecial(SpecialType, iWinnerIndex, gObj[iWinnerIndex].MapNumber, gObj[iWinnerIndex].X, gObj[iWinnerIndex].Y);
}
...
int CChaosCastle::RewardZettoMoney(int iChaosCastleIndex)
{
	...
	int i;
	for(i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
...
int CChaosCastle::GetFloor(int MapNumber)
{
	int Floor = -1;

	switch( MapNumber )
	{
	case 18:	Floor = 0; break;
	case 19:	Floor = 1; break;
	case 20:	Floor = 2; break;
	case 21:	Floor = 3; break;
	case 22:	Floor = 4; break;
	case 23:	Floor = 5; break;
	case 53:	Floor = 6; break;
	}

return Floor;
}

int CChaosCastle::GetMap(int iFloorIndex)
{
	int Map = 0;
