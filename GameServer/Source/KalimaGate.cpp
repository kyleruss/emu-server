...
BOOL CKalimaGate::CreateKalimaGate(int iIndex, BYTE btLevel, BYTE cTX, BYTE cTY)
{
	...
	__try
	{
		...
		if ( IF_MAP_RANGE(gObj[iIndex].MapNumber) )
		{
			return false;
		}
...
{
	if ( gObj[iIndex].Class == CLASS_MAGUMSA || gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_FIGHTER )	// DarkLord and MagicGLadiaro
	{
...
{
	if ( gObj[iIndex].Class == CLASS_MAGUMSA || gObj[iIndex].Class == CLASS_DARKLORD )	// DarkLord and MagicGLadiaro
	{
		for ( int i =0;i<KALIMA_FLOORS;i++)
		{
			if ( gObj[iIndex].Level >= g_sttKALIMAGATE_LEVEL[i].iLOWER_BOUND_MAGUMSA )
