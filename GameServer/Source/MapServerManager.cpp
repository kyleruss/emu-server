...
BOOL CMapServerManager::LoadData(char* lpszFileName)
{
	...
	xml_parse_result Result = Document.load_file(lpszFileName);
	// ----
	if( Result.status != status_ok )
	{
		MsgBox("[MapServerManager] Loading fail [%s]", lpszFileName);
		return FALSE;
	}
	// ----
	xml_node MapServer	= Document.child("mapserver");
	xml_node ServerList	= MapServer.child("serverlist");
	xml_node MoveList	= MapServer.child("movelist");
	xml_node Node;
	// ----
	for( Node = ServerList.child("server"); Node; Node = Node.next_sibling())
	{
		short ServerCode = Node.attribute("servercode").as_int(-1);
		short ServerGroup = Node.attribute("group").as_int();
		int InitSet = Node.attribute("init").as_int();
		WORD ServerPort = Node.attribute("port").as_int();
		// ----
		if( ServerCode < 0 )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : sSVR_CODE < 0 (SVR:%d) - 1", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		if( InitSet != -1 && InitSet != 0 && InitSet != 1 )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : iInitSetting Value:%d (SVR:%d) - 1", InitSet, ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		if( CHECK_LIMIT(ServerGroup, MAX_MAP_GROUPS) == FALSE )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : Map Server Group Index (IDX:%d)", ServerGroup);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		if( this->m_iMAPSVR_GROUP_COUNT[ServerGroup] >= MAX_MAP_SUBGROUPS )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : No Space to Save SvrInfo (SVR:%d)", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		_MAPSVR_DATA* lpMapSvrData = NULL;
		lpMapSvrData = &this->m_MAPSVR_DATA[ServerGroup][this->m_iMAPSVR_GROUP_COUNT[ServerGroup]];
		lpMapSvrData->Clear(InitSet);
		lpMapSvrData->m_bIN_USE = TRUE;
		lpMapSvrData->m_sSVR_CODE = ServerCode;
		lpMapSvrData->m_btMAPSVR_GROUP = (BYTE)ServerGroup;
		lpMapSvrData->m_wPORT = ServerPort;
		memcpy(lpMapSvrData->m_cIPADDR, Node.attribute("ip").as_string(), 16);
		lpMapSvrData->m_cIPADDR[15] = 0;
		this->m_mapSVRCODE_MAP.insert(std::pair<int, _MAPSVR_DATA*>(ServerCode, lpMapSvrData));
		this->m_iMAPSVR_GROUP_COUNT[ServerGroup]++;
	}
	// ----
	for( Node = MoveList.child("move"); Node; Node = Node.next_sibling())
	{
		short ServerCode = Node.attribute("servercode").as_int(-1);
		BYTE MoveState = Node.attribute("movestate").as_int(0);
		WORD MapNumber = Node.attribute("map").as_int(-1);
		short DestServerCode = Node.attribute("destservercode").as_int(-1);
		_MAPSVR_DATA* lpMapSvrData = NULL;
		// ----
		if( ServerCode < 0 )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : sSVR_CODE < 0 (SVR:%d) - 2", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		if( DestServerCode < -2 )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : sDEST_SVR_CODE < -1 (SVR:%d, DEST_SVR:%d) - 2", ServerCode, DestServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		std::map<int, _MAPSVR_DATA*>::iterator it = this->m_mapSVRCODE_MAP.find(ServerCode);
		// ----
		if( it == this->m_mapSVRCODE_MAP.end() )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : sSVR_CODE wasn't registered (SVR:%d)", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		...
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : lpMapSvrData == NULL (SVR:%d)", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		...
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : lpMapSvrData->m_bIN_USE == FALSE (SVR:%d)", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		if( lpMapSvrData->m_sSVR_CODE != ServerCode )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : lpMapSvrData->m_sSVR_CODE != sSVR_CODE (SVR:%d)", ServerCode);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		...
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : lpMapSvrData->m_sSVR_CODE != sSVR_CODE (SVR:%d, OPT:%d)", ServerCode, MoveState);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----
		if( CHECK_LIMIT(MapNumber, MAX_NUMBER_MAP) == FALSE )
		{
			MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : Map Number is out of bound (SVR:%d, MAP:%d)", ServerCode, MapNumber);
			LeaveCriticalSection(&this->m_critSVRCODE_MAP);
			return FALSE;
		}
		// ----			
		...
		{
			case 0:
				lpMapSvrData->m_sMAP_MOVE[MapNumber] = DestServerCode;
				break;
			case 1:
				lpMapSvrData->m_sMAP_MOVE[MapNumber] = -3;
				break;
			default:
				MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : lpMapSvrData->m_sSVR_CODE != sSVR_CODE (SVR:%d, OPT:%d)", ServerCode, MoveState);
				break;
	...
	{
		MsgBox("[MapServerMng] CMapServerManager::LoadData() - file load error : This GameServerCode (%d) doesn't Exist at file '%s' != sSVR_CODE", gGameServerCode, lpszFileName);
		LeaveCriticalSection(&this->m_critSVRCODE_MAP);
		return FALSE;
	}
	// ----
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( type == 0 )
				{
					...
					lpMapSvrData->m_btMAPSVR_GROUP = (BYTE)sMAPSVR_GROUP;
				...
				else if ( type == 1 )
				{
					...
					Token = GetToken();
					btNotMoveOption = (BYTE)TokenNumber;

					Token = GetToken();
					wMapNum = (WORD)TokenNumber;

					Token = GetToken();
					sDEST_SVR_CODE = (short)TokenNumber;
