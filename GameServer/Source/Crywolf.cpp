...
CCrywolf::CCrywolf()
{
	...
	memset(m_iMonsterGroupNumberArray, 0, sizeof( m_iMonsterGroupNumberArray ) );
}
...
void CCrywolf::DelAllData()
{
	...
	memset(this->m_iMVPScoreTable, 0, sizeof( m_iMVPScoreTable ));
	memset(this->m_iMVPRankScoreTable, 0, sizeof( m_iMVPRankScoreTable ));
	memset(this->m_iMVPRankExpTable, 0, sizeof( m_iMVPRankExpTable ));
...
void CCrywolf::SetCrywolfCommonNPC(int iOccupationState)
{
	for ( int i=0;i<this->m_ObjCommonNPC.m_iObjCount ; i++)
	{
		...
		if( iOccupationState == CRYWOLF_STATE_NOTIFY_2 ||
			iOccupationState == CRYWOLF_STATE_NOTIFY_1 )
		{
			gObjAddBuffEffect(lpObj,BUFF_CRYWOLF_TRANSPARENCY,0,0,0,0,-10);
		}
		else if( iOccupationState == CRYWOLF_STATE_NONE )
		{
			gObjRemoveBuffEffect(lpObj,BUFF_CRYWOLF_TRANSPARENCY);
		}
	}
}
...
void CCrywolf::ProcState_START()
{
	...
	if ( (GetTickCount() - this->m_dwCrywolfStartProcTick) > (DWORD)(this->m_iBossTurnUpTime * 1000) &&
		 !this->m_bTurnUpBoss )
	{
	...
	if ( (GetTickCount() - this->m_dwCrywolfStartProcTick) > (DWORD)(this->m_iMonsterGroupChangeAITime * 1000) &&
		 !this->m_bChangeAI )
	{
...
void CCrywolf::NotifyCrywolfStateLeftTime()
{
	...
	LogAddC(3, "[ Crywolf ] War LeftTime - (%02d:%02d)",
	...
	for ( int i=0;i<OBJ_MAXMONSTER;i++)
	{
		...
		switch ( gObj[i].Class )
		{
			case 340:
				...
				break;
			case 349:
				pMsg.iMonster1HP = (int)((gObj[i].Life * 100.0f) / (gObj[i].MaxLife + 1.0f));
				break;
		}
	}
...
using namespace std;
	...
	set<LPOBJ,CCrywolfScoreSort> HeroSet;
	...
	set<LPOBJ, CCrywolfScoreSort>::iterator _Itor = HeroSet.begin();
...
void CCrywolf::ApplyCrywolfDBInfo(int iCrywolfState, int iOccupationState)
{
	...
	LogAddTD("[ Crywolf ][ ApplyDBInfo] OccupationState : %d",
		iOccupationState);
}
...
int CCrywolf::CalcGettingRewardExp(int iUserIndex, int iMVPRank)
{
	...
	CheckItemOptForGetExpEx(&gObj[iUserIndex],iRewardExp,1);
